...
export function useSchemaDiffConfig() {
  const activeConfig = computed(
    () => configs.value.find((c) => c.id === activeConfigId.value) ?? configs.value[0] ?? null,
  );
  ...
  function createConfig(name: string, base?: SchemaDiffConfig, dbType?: string): SchemaDiffConfig {
    const id = uuid();
    const config = base
      ? { ...base, id, name, createdAt: Date.now(), updatedAt: Date.now() }
      : createEmptyConfig(id, name);
    if (dbType && !base) {
      config.options = getDefaultOptionsForDbType(dbType);
    }
  ...
  function saveToHistory(config: SchemaDiffConfig) {
    ...
    const existingIndex = recentConfigs.value.findIndex(
      (c) =>
        c.sourceConnectionId === config.sourceConnectionId &&
        c.sourceDatabase === config.sourceDatabase &&
        c.sourceSchema === config.sourceSchema &&
        c.targetConnectionId === config.targetConnectionId &&
        c.targetDatabase === config.targetDatabase &&
        c.targetSchema === config.targetSchema,
    );
