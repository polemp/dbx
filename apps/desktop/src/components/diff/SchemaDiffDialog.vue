...
watch(
  ...
  ([srcConn, srcDb, srcSchema, tgtConn, tgtDb, tgtSchema]) => {
    updateActiveConfigConnection({
...
async function handleCompare() {
  ...
  try {
    ...
    diffObjects.value = convertToSchemaDiffObjects(
      result.diffs,
...
function handleToggleGroup(operationType: DiffOperationType) {
  diffGroups.value = diffGroups.value.map((g) =>
    g.operationType === operationType ? { ...g, expanded: !g.expanded } : g,
  );
}
    ...
    >
      ...
      >
        <div class="bg-card border rounded-lg shadow-lg w-[500px] max-h-[80vh] overflow-auto p-4">
          ...
          <SchemaDiffOptionsPanel
            :options="activeConfig?.options ?? getDefaultOptionsForDbType(getDbType())"
            :option-tree="optionTree"
