...
<template>
  <div class="flex items-center gap-2">
    ...
    <Button
      ...
      size="icon"
      class="h-8 w-8"
      :title="t('schemaDiff.renameConfig')"
      @click="startRename(activeConfig)"
    >
      <Copy class="h-4 w-4" />
    </Button>
    ...
    <Button
      ...
      size="icon"
      class="h-8 w-8"
      :title="t('schemaDiff.duplicateConfig')"
      @click="emit('duplicate', activeConfig.id)"
    >
      <FileDown class="h-4 w-4" />
    </Button>

    <Button
      v-if="activeConfig && configs.length > 1"
      variant="outline"
      size="icon"
      class="h-8 w-8"
      :title="t('schemaDiff.deleteConfig')"
      @click="activeConfig && emit('delete', activeConfig.id)"
    >
    ...
    <Button
      ...
      size="icon"
      class="h-8 w-8"
      :title="t('schemaDiff.importConfig')"
    ...
    <Button
      ...
      size="icon"
      class="h-8 w-8"
      :title="t('schemaDiff.exportAllConfigs')"
      @click="onExportAll"
    >
      <FileDown class="h-4 w-4" />
    </Button>
      ...
      <DialogContent class="sm:max-w-[500px]">
        ...
        <div class="py-4 space-y-4">
          ...
          <div>
            ...
            <textarea
              v-model="importValue"
              class="mt-2 w-full min-h-[120px] rounded-md border border-input bg-background px-3 py-2 text-sm"
              :placeholder="t('schemaDiff.importPlaceholder')"
            />
          </div>
