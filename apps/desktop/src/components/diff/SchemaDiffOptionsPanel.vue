  ...
  <div class="flex flex-col h-full">
    <div class="flex-1 overflow-auto space-y-1">
      <template v-for="item in optionTree" :key="item.id">
        <div class="space-y-1">
          <div
            class="flex items-center gap-2 py-1 px-2 rounded hover:bg-muted/50 cursor-pointer"
            @click="toggleItem(item)"
          >
            <div :class="getItemClasses(getChildState(item))">
              <svg
                v-if="getChildState(item) === 'checked'"
                class="h-3 w-3"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
              ...
              <svg
                v-else-if="getChildState(item) === 'indeterminate'"
                class="h-3 w-3"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
          ...
          <div v-if="item.children" class="ml-6 space-y-1">
            ...
            >
              <div :class="getItemClasses(getChildState(child))">
                <svg
                  v-if="getChildState(child) === 'checked'"
                  class="h-3 w-3"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
