...
async function loadDatabases(connectionId: string, side: "source" | "target") {
  ...
  try {
    ...
    const dbNames = Array.isArray(dbs)
      ? dbs.map((db: any) => (typeof db === "string" ? db : db.name || db.database))
      : [];
    if (side === "source") {
...
<template>
  <div class="space-y-4">
      ...
      <div class="space-y-3">
        ...
        <div class="space-y-1.5">
          ...
          <Select
            ...
            @update:model-value="(v: any) => $emit('update:sourceConnectionId', String(v))"
          >
      ...
      <div class="space-y-3">
        ...
        <div class="space-y-1.5">
          ...
          <Select
            ...
            @update:model-value="(v: any) => $emit('update:targetConnectionId', String(v))"
          >
            <SelectTrigger class="h-8 text-xs">
              <div class="flex items-center gap-2">
                <DatabaseIcon
    ...
    <div class="flex items-center gap-2 p-3 rounded-lg border bg-muted/20">
      <input
        id="schema-diff-ignore-comments"
        :checked="ignoreComments"
        type="checkbox"
        class="accent-primary"
        @change="$emit('update:ignoreComments', ($event.target as HTMLInputElement).checked)"
      />
        ...
        <SelectContent>
          <SelectItem v-for="config in recentConfigs" :key="config.id" :value="config.id" class="pr-8">
            <div class="flex items-center justify-between w-full gap-2">
              ...
              <button
                class="shrink-0 p-1 rounded hover:bg-destructive/10 hover:text-destructive transition-colors"
                @click.stop="$emit('deleteHistoryConfig', config.id)"
              >
                <X class="w-3 h-3" />
              </button>
            </div>
