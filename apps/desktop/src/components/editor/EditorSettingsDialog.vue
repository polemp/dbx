          ...
          <div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-1 pr-2">
            ...
            <section v-else-if="activeSettingsTab === 'navigation'" class="flex flex-col gap-5 py-2">
              ...
              <div class="space-y-2 rounded-md border bg-muted/20 px-3 py-2">
                <div class="flex items-center gap-2">
                  <Label for="routine-source-open-mode">{{ t("settings.routineSourceOpenMode") }}</Label>
                  <HelpTooltip :label="t('settings.routineSourceOpenMode')">
                    {{ t("settings.routineSourceOpenModeDialogDescription") }}
                  </HelpTooltip>
                </div>
                <Select :model-value="editRoutineSourceOpenMode" @update:model-value="(value) => (editRoutineSourceOpenMode = value as 'query-tab' | 'dialog')">
                  <SelectTrigger id="routine-source-open-mode" class="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="query-tab">{{ t("settings.routineSourceOpenModeQueryTab") }}</SelectItem>
                    <SelectItem value="dialog">{{ t("settings.routineSourceOpenModeDialog") }}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div class="flex items-center justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2">
